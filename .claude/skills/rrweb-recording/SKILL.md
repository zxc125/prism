...
| [packages/observer-sdk](packages/observer-sdk) | npm 包 `@prism/observer-sdk`：`Sink` 接口 + 类型、`HttpSink`（含 `useSessionId` 注入）/`IndexedDBSink`、信号 hook（`installSignalHooks`）、`SegmentRecorder`、`init()`；self-obs / web SDK / tauri plugin 共用 |
...
- `pnpm --filter @prism/observer-sdk build` 产 `dist/index.js`；`examples/web-demo` `tsc --noEmit` 通过。
