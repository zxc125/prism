...
| [packages/observer-tauri](packages/observer-tauri) | npm 包 `@prism-obs/observer-tauri`：`initTauri()` 采集端驱动--监听插件 `recording-session`/`segment`/`observer-lifecycle` 事件驱动 `SegmentRecorder`，hidden/focus 经 `HttpSink.appendLifecycle` 上报；主窗口 `autoStart` 建 session + `bind_session` 广播。P16 起 `mode?: "remote"\|"local"` 双模式：local 用包内 `TauriSink`（`src/sink.ts`）+ 插件 Rust 本地落盘，controller 增 `listSessions`/`exportSession` |
...
- P16 3.2 Local 端到端 ✅ 已实测（2026-09-09）：tauri-demo `VITE_OBSERVER_MODE=local` 录制 -> 导出 bundle -> console 导入回放全链路还原（步骤见 docs/测试/P16-测试流程.md 场景 4）。
