...
**进度**：P1-P12 全部 ✅；P13 官网文档站 Phase 1 ✅（[方案](docs/架构/官网文档站（方案）.md) · [P13](docs/阶段路径/P13-官网文档站.md)）；P14 录制事件量控 🚧（A/B + 发版 + 实测 + 3.3 手册 ✅，余 2.3 手动对照，[P14](docs/阶段路径/P14-录制事件量控.md)）；P15 手册小白化 ✅（[P15](docs/阶段路径/P15-手册小白化.md)，全链路 live run 留手动场景）；P16 外部应用本地落盘 ✅（[P16](docs/阶段路径/P16-外部应用本地落盘.md)，实测 + 发版 crates 0.2.0 / npm 0.3.0）；P17 段控制上收与性能加固 🚧（上游全部 ✅ 含发版 crates 0.2.1 / npm 0.3.0·0.4.0，余下游对接测试——bond/frontend 升级三包 + `.prism-block` 扩行容器 + 开段挪 idle 时机，[P17](docs/阶段路径/P17-段控制上收与性能加固.md)）。下表只列「是什么 + 1-2 个核心指针 + 验证状态」，实现细节归各阶段文档（零信息丢失，已逐一核对指针落在对应 P 文档内）。
