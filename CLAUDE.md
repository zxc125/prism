...
**进度**：P1-P12 全部 ✅；P13 官网文档站 Phase 1 ✅（[方案](docs/架构/官网文档站（方案）.md) · [P13](docs/阶段路径/P13-官网文档站.md)）；P14 录制事件量控 🚧 Phase A/B 落地（[P14](docs/阶段路径/P14-录制事件量控.md)，手动对照与发版待做）。下表只列「是什么 + 1-2 个核心指针 + 验证状态」，实现细节归各阶段文档（零信息丢失，已逐一核对指针落在对应 P 文档内）。
...
| P14 | 录制事件量控 | [recording-profile.ts](packages/observer-sdk/src/recording-profile.ts) 三档预设 + `domBlocks` 免录区 · [方案](docs/架构/录制事件量控（方案）.md) · [测试流程](docs/测试/P14-测试流程.md) | 🚧 Phase A/B：三包 typecheck 0 错 + `pnpm build` ✓；手动对照/发版待做 |
