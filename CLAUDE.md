...
`pnpm build` 会先执行 `vue-tsc --noEmit` - 类型错误会导致构建失败。JS 测试仅 [packages/observer-sdk](packages/observer-sdk) 有（vitest，`pnpm --filter @prism-obs/observer-sdk test`，P17 引入）；Rust 测试为各 crate 的 `cargo test`。
...
2. **观测外部** - Web SDK（npm，单 tab）+ `tauri-plugin-observer`（多窗口对齐）：默认 HTTP sink -> console 本地 server；Tauri 侧可 opt-in 本地落盘（Local 模式泛化给外部应用，bundle 导出闭环回 console，见 [P16](docs/阶段路径/P16-外部应用本地落盘.md)）。
...
阶段路径：P1 分析端页面改造 -> P2 诊断信号采集 -> P3 sink 抽象 -> P4 Web SDK -> P5 Tauri Plugin -> P6 导出/标注/分享 -> P7 离线采集与 bundle 契约 -> P8 云端 server 抽取 -> P9 多租户与运营加固 -> P10 console 2.0 重设计 + 浏览器化 -> P11 官网与品牌 -> P12 官网 i18n -> P13 官网文档站 -> P14 录制事件量控 -> P15 手册小白化 -> P16 外部应用本地落盘 -> P17 段控制上收与性能加固 -> P18 导出to-file变体 -> P19 大文件回放性能 -> P20 外部落盘根自定义 -> P21 会话列表表格化与视觉统一 -> P22 会话用户归属与会话级启停。各阶段实现细节见 [docs/阶段路径/](docs/阶段路径/)，架构方案见 [docs/架构/](docs/架构/)，品牌见 [docs/品牌/](docs/品牌/)。

**进度**：P1-P12 全部 ✅；P13 官网文档站 Phase 1 ✅（[方案](docs/架构/官网文档站（方案）.md) · [P13](docs/阶段路径/P13-官网文档站.md)）；P14 录制事件量控 🚧（A/B + 发版 + 实测 + 3.3 手册 ✅，余 2.3 手动对照，[P14](docs/阶段路径/P14-录制事件量控.md)）；P15 手册小白化 ✅（[P15](docs/阶段路径/P15-手册小白化.md)，全链路 live run 留手动场景）；P16 外部应用本地落盘 ✅（[P16](docs/阶段路径/P16-外部应用本地落盘.md)，实测 + 发版 crates 0.2.0 / npm 0.3.0）；P17 段控制上收与性能加固 ✅（[P17](docs/阶段路径/P17-段控制上收与性能加固.md)，上游含发版 crates 0.2.1（+0910 补丁 0.2.2：`export_session` async 化）/ npm 0.3.0·0.4.0 + 下游对接验收完成 2026-09-10）；P18 导出 to-file 变体 ✅（[P18](docs/阶段路径/P18-导出to-file变体.md)，发版 crates 0.3.0 / npm 0.5.0，2026-09-10）；P19 大文件回放性能 ✅（[P19](docs/阶段路径/P19-大文件回放性能.md)，落地 + 运行时验收通过 2026-09-10；Phase D 延后）；P20 外部落盘根自定义 ✅（[P20](docs/阶段路径/P20-外部落盘根自定义.md)，subagent 独立复核零问题 + 发版 crates 0.4.0，2026-09-17；bond-sat 依赖锁定回改 0.4.0 待其确认）；P21 会话列表表格化与视觉统一 ✅（[P21](docs/阶段路径/P21-会话列表表格化与视觉统一.md)，el-table-v2 通道台账 + 时长语义修复 + EP 字体统一 + 批量删除，subagent 独立复核零阻断，2026-09-17）；P22 会话用户归属与会话级启停 ✅（[P22](docs/阶段路径/P22-会话用户归属与会话级启停.md)，sdk `SessionMeta.user` + controller 会话级启停 + 插件 meta 注入/stop 只补 endedAt + console 用户列，/regress 复核与用户实测 + 发版 npm 0.4.0·0.6.0 / crates 0.5.0，2026-09-18；bond-sat 依赖锁回改待其确认）。下表只列「是什么 + 1-2 个核心指针 + 验证状态」，实现细节归各阶段文档（零信息丢失，已逐一核对指针落在对应 P 文档内）。
...
| P21 | 会话列表表格化与视觉统一 | [SessionList.vue](src/components/sessions/SessionList.vue) el-table-v2 通道台账（选择列/来源色点/未结束徽标/appId 列/批量删除）+ [format.ts](src/components/common/format.ts) `sessionDur` 废 now 兜底 + theme.css `--el-font-family` 覆写 · [方案](docs/架构/会话列表表格化与视觉统一（方案）.md) · [测试流程](docs/测试/P21-测试流程.md) | ✅ `pnpm build` 0 错 + dist 含 table-v2 样式 + subagent 独立复核零阻断（named ellipsis 失效已修）；SessionCard 删、渲染并入 List（D5 偏离已记档） |
| P22 | 会话用户归属与会话级启停 | sdk `SessionMeta.user` + [observer-tauri](packages/observer-tauri/src/index.ts) controller `startSession(meta)`/`stopSession`（幂等收口 + D6a 重绑）+ 插件 `start_session(meta)` + stop 直用 `finalize_session` 只补 endedAt + console 会话列表「用户」列（D8 后并入 1.9）· [方案](docs/架构/会话用户归属与会话级启停（方案）.md) · [决策](docs/决策/会话用户归属与会话级启停.md) · [测试流程](docs/测试/P22-测试流程.md) | ✅ 插件 13 + storage 33 + server 23+9 + vitest 3 + console/site build ✅ + 两轮 subagent 复核（Remote 幂等 end 竞态收窄记档）+ 用户实测 · 发版 sdk 0.4.0 / tauri 0.6.0 / crates 0.5.0，registry 可拉取（2026-09-18；bond-sat 依赖锁回改待其确认） |
...
recordings/<sessionId>/
  session.json          # { id, startedAt, endedAt?, source?, appId?, user?, name?, note?, tags?, importedAt? }
...
- **签名元素**：Sessions = 会话浏览器（表列行 + 来源色点，P21 起表格化——措辞保持形态中性，勿再绑定具体组件形态）；Live = 源机架（多通道控制室输入 bay，本机通道承袭 REC 脉冲 DNA）；Player = 多轨时间轴 + 真实播放头 + 诊断信号流；TopBar = tenant 上下文 + 配额余量条。新视图应贡献自己的一个签名元素，而非复刻通用模板。
